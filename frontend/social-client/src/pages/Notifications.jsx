...
function NotificationItem({ type, user, handle, time, content, isRead }) {
  ...
  return (
    <div className={`border-b border-gray-200 p-4 hover:bg-gray-50 transition-colors flex gap-3 ${!isRead ? 'bg-blue-50' : ''}`}>
      {/* Icon */}
      {getIcon()}
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1">
            ...
            {content && (
              <div className="mt-2 text-sm text-gray-700 bg-gray-100 border border-gray-200 rounded-lg p-3">
                {content}
              </div>
            )}

            {type === "follow" && (
              <button className="mt-3 bg-black text-white text-sm px-4 py-1.5 rounded-full hover:bg-gray-800 transition-colors">
                Follow Back
              </button>
            )}
          </div>
